...
namespace MissionPlanner.Plugins.GimbalControlRC
{
    public partial class GimbalControlButtons : Form
    {
        ...
        private double XReverse = -1, YReverse = -1;
        ...
        void Joystick_Poll(object Sender, EventArgs e)
        {
            ...
            if (state != null)
            {
                ...
                {
                    if (Math.Abs(state.Y - 104) > 10)
                        position[0] = ((state.Y - 32768.0f) / 65536.0f * -2.0f) * 90.0f * YReverse;
                    else
                        position[0] = 0.0f;
                    if (Math.Abs(state.X - 112) > 10)
                        position[2] = ((state.X - 32768.0f) / 65536.0f * 2.0f) * 90.0f * XReverse;
                    else
                        position[2] = 0.0f;
                    if (state.Z > 40000)     //Logitech 32768 max 65536
                        zoomIn_Click(null, null);
                    if (state.Z < 82)     //Logitech 32768 max 65536
                        zoomOut_Click(null, null);
